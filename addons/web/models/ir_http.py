...
class Http(models.AbstractModel):
    ...
    def session_info(self):
        ...
        logging.info(">>>>>>>>>>>>>>>>request.session.sid>>>DURING LOGIN>>>>>>>>>>%s",request.session.sid)
        logging.info(">>>>>>>>>>>>>>>>request.httprequest.cookies>>>DURING LOGIN>>>>>>>>>>%s",request.httprequest.cookies)
